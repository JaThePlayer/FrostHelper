...
namespace FrostHelper.Entities
{
	public class LavaShape : Component
	{
		...
		public void Resize(int step)
		{
			...
			int num = (int)(GetFullLength() / (float)SurfaceStep * 2f + 4f);
			...
			for (int i = 0; i < bubbles.Length; i++)
			{
				...
				bubbles[i].Speed = (float)Calc.Random.Range(4, 12);
		...
		public override void Update()
		{
			...
			for (int j = 0; j < surfaceBubbles.Length; j++)
			{
				if (surfaceBubbles[j].X >= 0f)
				{
					...
					if (surfaceBubbles[j].Frame >= (float)surfaceBubbleAnimations[(int)surfaceBubbles[j].Animation].Count)
					{
						surfaceBubbles[j].X = -1f;
					}
				}
		...
		private float Wave(int step, float length)
		{
			...
			float num2 = (OnlyMode != OnlyModes.None) ? 1f : (Calc.ClampedMap((float)num, 0f, length * 0.1f, 0f, 1f) * Calc.ClampedMap((float)num, length * 0.9f, length, 1f, 0f));
		...
		private void Edge(ref int vert, Vector2 a, Vector2 b, float fade, float insetFade, bool continueFromPrevious, bool createAfter)
		{
			...
			float num3 = length / (float)SurfaceStep;
			Vector2 perpendicular = (b - a).SafeNormalize().Perpendicular();
			int i = 1;
			while (i <= num3)
			{
				Vector2 value = Vector2.Lerp(a, b, (float)(i - 1) / num3);
				float num5 = Wave(i - 1, length);
				Vector2 outer = value - perpendicular * num5;
				Vector2 value2 = Vector2.Lerp(a, b, (float)i / num3);
				float num6 = Wave(i, length);
				Vector2 inner = value2 - perpendicular * num6;
				Vector2 value3 = Vector2.Lerp(a, b, Calc.ClampedMap((float)(i - 1) / num3, 0f, 1f, num2, 1f - num2));
				Vector2 value4 = Vector2.Lerp(a, b, Calc.ClampedMap((float)i / num3, 0f, 1f, num2, 1f - num2));
		...
		public override void Render()
		{
			...
			for (int j = 0; j < surfaceBubbles.Length; j++)
			{
				if (surfaceBubbles[j].X >= 0f)
				{
					MTexture mtexture2 = surfaceBubbleAnimations[(int)surfaceBubbles[j].Animation][(int)surfaceBubbles[j].Frame];
					int num = (int)(surfaceBubbles[j].X / (float)SurfaceStep);
					float y = surfaceBubbles[j].Y;//1f - Wave(num, Width);
					mtexture2.DrawJustified(value + new Vector2((float)(num * SurfaceStep), y), new Vector2(0.5f, 1f), SurfaceColor);
				}
			}
		}
