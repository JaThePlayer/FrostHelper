...
internal sealed class OnExpressionChangedActivator : BaseActivator {
    ...
    {       
        Add(new ExpressionListener<object>(data.GetCondition("expression"), OnExprChanged, activateOnStart: false));
        Active = true;
        Visible = false;
        Collidable = false;
    }

    private static void OnExprChanged(Entity self, Maybe<object> prev, object curr) {
